...
# =================== OWNER - ANL0KE =================== #
ALIVE_NAME = Config.ALIVE_NAME if Config.ALIVE_NAME else "@svillethon"
    ...
    if LOAD_MYBOT == "False":
        ...
        if BOT_PIC:
            await tgbot.send_message(event.chat_id, BOT_PIC,
                                  ...
                                  buttons=[
                                      [Button.inline(
                                          "⚜️ svillethon - 𝙐𝙎𝙀𝙍𝘽𝙊𝙏 ⁦⚜️", data="Icss")]
                                  ])
        else:
...
async def logs(event):
    ...
    await tgbot.send_file(
        ...
        "logs.txt",
        reply_to=event.id,
        caption="**Heroku** svillethon Logs",
        buttons=[
            [Button.url("💠 𝙑𝙄𝙀𝙒 𝙊𝙉𝙇𝙄𝙉𝙀 💠", f"{url}")],
            [Button.url("💠 𝘾𝙍𝘼𝙎𝙃𝙀𝘿 💠", "t.me/svillethon")]
        ])
...
async def settings(event):
    ...
    await tgbot.send_message(event.chat_id,
                             ...
                             buttons=[
                                     [Button.inline(
                                         "⚜️ احصل على بوت سفيلثون بنفسك ⚜️", data="deployme")]
                             ])
...
async def settings(event):
    await event.delete()
    await event.reply(f"⌔∮ انا سفيلثون مساعد {mention}. الشخصي تستطيع الاتصال بي عن طريق هذا البوت",
                             buttons=[
                                     [Button.inline(
                                         "⚜️ حصل على بوت سفيلثون بنفسك ⚜️", data="deployme")]
                             ])
...
async def settings(event):
    await event.edit("** ⌔∮ هنا ستجد رابط سفيلثون ورابط التنصيب ايضا**:",
                     buttons=[
                         [Button.url("🔗 رابط السورس🔗", url="https://github.com/DRACULA-22/svillethon")],
                         [Button.url("🔗 رابط التنصيب 🔗", url="https://dashboard.heroku.com/new?template=https://github.com/DRACULA-22/svillethon")],
                         [Button.url("✨ الدعم ✨",
                                     url="https://t.me/svillethon")]
                     ])
...
async def settings(event):
    if event.sender_id == OWNER_ID:
        ...
        await event.answer("انت لا تستطيع استخدام البوت احصل على بوتك من @svillethon", alert=True)
